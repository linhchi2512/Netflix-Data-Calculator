...
# Initialize an empty DataFrame to store all saved results
if "results_data" not in st.session_state:
    st.session_state["results_data"] = pd.DataFrame()

# Initialize a placeholder for the currently calculated result
...
def main():
    ...
    if st.button("Calculate"):
        ...
        st.session_state["calculated_results"] = pd.DataFrame([results])

        st.subheader("Calculated Results:")
        st.dataframe(st.session_state["calculated_results"])

    # Display and Save Results if available
    if st.session_state["calculated_results"] is not None:
        ...
        if st.button("Save Results"):
            ...
            st.session_state["results_data"] = pd.concat(
                [st.session_state["results_data"], st.session_state["calculated_results"]],
                ignore_index=True
            )
            st.session_state["calculated_results"] = None  # Clear the calculated results after saving
